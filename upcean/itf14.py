...
'''
    ...
    $FileInfo: itf14.py - Last Update: 02/28/2012 Ver. 2.2.5 RC 1 - Author: cooldude2k $
'''
...
def create_itf14(upc,outfile="./itf14.png",resize=1,hidecd=False):
	...
	if(len(upc_matches)<=0):
		return False;
	upc_preimg = cairo.ImageSurface(cairo.FORMAT_RGB24, 44 + upc_size_add, 62);
	upc_img = cairo.Context (upc_preimg);
	upc_img.set_antialias(cairo.ANTIALIAS_NONE);
	upc_img.rectangle(0, 0, 44 + upc_size_add, 62);
	...
	while (NumTxtZero < len(upc_matches)):
		...
		drawColorText(upc_img, 11, LineTxtStart, 59, ArrayDigit[0], [0, 0, 0]);
		LineTxtStart += 9;
		drawColorText(upc_img, 11, LineTxtStart, 59, ArrayDigit[1], [0, 0, 0]);
		LineTxtStart += 9;
		NumTxtZero += 1;
	...
	drawColorLine(upc_img, 9, 4, 9, 47, [256, 256, 256]);
	...
	drawColorLine(upc_img, 13, 4, 13, 47, [256, 256, 256]);
	drawColorLine(upc_img, 14, 4, 14, 47, [256, 256, 256]);
	drawColorLine(upc_img, 15, 4, 15, 47, [256, 256, 256]);
	drawColorLine(upc_img, 16, 4, 16, 47, [256, 256, 256]);
	drawColorLine(upc_img, 17, 4, 17, 47, [0, 0, 0]);
	drawColorLine(upc_img, 18, 4, 18, 47, [256, 256, 256]);
	drawColorLine(upc_img, 19, 4, 19, 47, [0, 0, 0]);
	drawColorLine(upc_img, 20, 4, 20, 47, [256, 256, 256]);
	NumZero = 0; 
	LineStart = 21; 
	LineSize = 47;
	while (NumZero < len(upc_matches)):
		...
		while (InnerUPCNum < len(left_text_color)):
			...
			InnerUPCNum += 1;
		NumZero += 1;
	drawColorLine(upc_img, 21 + upc_size_add, 4, 21 + upc_size_add, 47, [0, 0, 0]);
	drawColorLine(upc_img, 22 + upc_size_add, 4, 22 + upc_size_add, 47, [0, 0, 0]);
	drawColorLine(upc_img, 23 + upc_size_add, 4, 23 + upc_size_add, 47, [0, 0, 0]);
	drawColorLine(upc_img, 24 + upc_size_add, 4, 24 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 25 + upc_size_add, 4, 25 + upc_size_add, 47, [0, 0, 0]);
	...
	drawColorLine(upc_img, 28 + upc_size_add, 4, 28 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 29 + upc_size_add, 4, 29 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 30 + upc_size_add, 4, 30 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 31 + upc_size_add, 4, 31 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 32 + upc_size_add, 4, 32 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 33 + upc_size_add, 4, 33 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 34 + upc_size_add, 4, 34 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 35 + upc_size_add, 4, 35 + upc_size_add, 47, [256, 256, 256]);
	drawColorLine(upc_img, 36 + upc_size_add, 4, 36 + upc_size_add, 47, [256, 256, 256]);
	...
	drawColorRectangle(upc_img, 0, 0, 43 + upc_size_add, 50, [0, 0, 0]);
	drawColorRectangle(upc_img, 1, 1, 41 + upc_size_add, 48, [0, 0, 0]);
	drawColorRectangle(upc_img, 2, 2, 39 + upc_size_add, 46, [0, 0, 0]);
	drawColorRectangle(upc_img, 3, 3, 37 + upc_size_add, 44, [0, 0, 0]);
	upc_imgpat = cairo.SurfacePattern(upc_preimg);
	scaler = cairo.Matrix();
	scaler.scale(1/int(resize),1/int(resize));
	upc_imgpat.set_matrix(scaler);
	upc_imgpat.set_filter(cairo.FILTER_BEST);
	new_upc_preimg = cairo.ImageSurface(cairo.FORMAT_RGB24, (44 + upc_size_add) * int(resize), 62 * int(resize));
