...
'''
    ...
    $FileInfo: ean2.py - Last Update: 3/27/2023 Ver. 2.8.13 RC 1 - Author: cooldude2k $
'''
...
def create_ean2_barcode_supplement(upc,outfile="./ean2_supplement.png",resize=1,hideinfo=(False, False, False),barheight=(48, 54),barwidth=(1, 1),textxy=(1, 1, 1),barcolor=((0, 0, 0), (0, 0, 0), (255, 255, 255)),imageoutlib="pillow"):
 ...
 if(pilsupport and imageoutlib=="pillow"):
  upc_preimg = Image.new("RGB", ((29 * barwidth[0]), barheight[1] + (9 * barwidth[1])));
  upc_img = ImageDraw.Draw(upc_preimg);
  upc_img.rectangle([(0, 0), ((29 * barwidth[0]), barheight[1] + (9 * barwidth[1]))], fill=barcolor[2]);
 ...
 while (NumZero < len(LeftDigit)):
  ...
  NumZero += 1;
 if(pilsupport and imageoutlib=="pillow"):
  new_upc_img = upc_preimg.resize(((29 * barwidth[0]) * int(resize), (barheight[1] + (9 * barwidth[1])) * int(resize)), Image.NEAREST);
  del(upc_img);
  del(upc_preimg);
 ...
 if(cairosupport and (imageoutlib=="cairo" or imageoutlib=="cairosvg")):
  ...
  if(outfileext=="SVG"):
   if(outfile is None):
    ...
    if(sys.version[0]>="3"):
     svgoutfile = BytesIO();
   new_upc_preimg = cairo.SVGSurface(svgoutfile, (29 * barwidth[0]) * int(resize), (barheight[1] + (9 * barwidth[1])) * int(resize));
  else:
   new_upc_preimg = cairo.ImageSurface(cairo.FORMAT_RGB24, (29 * barwidth[0]) * int(resize), (barheight[1] + (9 * barwidth[1])) * int(resize));
