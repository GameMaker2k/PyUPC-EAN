...
def create_ean2sup_barcode(upc,outfile="./ean2_supplement.png",resize=1,hideinfo=(False, False, False),barheight=(48, 54),barwidth=(1, 1),textxy=(1, 1, 1),barcolor=((0, 0, 0), (0, 0, 0), (255, 255, 255)),imageoutlib="pillow"):
 ...
 if(pilsupport and imageoutlib=="pillow"):
  upc_preimg = Image.new("RGB", ((29 * barwidth[0]), barheightadd + (9 * barwidth[1])));
  upc_img = ImageDraw.Draw(upc_preimg);
  upc_img.rectangle([(0, 0), ((29 * barwidth[0]), barheightadd + (9 * barwidth[1]))], fill=barcolor[2]);
 if(cairosupport and (imageoutlib=="cairo" or imageoutlib=="cairosvg")):
  if(outfileext=="SVG"):
   upc_preimg = cairo.SVGSurface(None, (29 * barwidth[0]), barheightadd + (9 * barwidth[1]));
  elif(outfileext=="PDF"):
   ...
   upc_preimg = cairo.ImageSurface(cairo.FORMAT_RGB24, (29 * barwidth[0]), barheightadd + (9 * barwidth[1]));
 ...
 while (NumZero < len(LeftDigit)):
  ...
  NumZero += 1;
 if(pilsupport and imageoutlib=="pillow"):
  new_upc_img = upc_preimg.resize(((29 * barwidth[0]) * int(resize), (barheightadd + (9 * barwidth[1])) * int(resize)), Image.NEAREST);
  del(upc_img);
  del(upc_preimg);
 ...
 if(cairosupport and (imageoutlib=="cairo" or imageoutlib=="cairosvg")):
  ...
  if(outfileext=="SVG" or outfileext=="PDF" or outfileext=="PS" or outfileext=="EPS"):
   ...
   if(outfileext=="SVG"):
    ...
    new_upc_preimg = cairo.ImageSurface(cairo.FORMAT_RGB24, (29 * barwidth[0]) * int(resize), (barheightadd + (9 * barwidth[1])) * int(resize));
  else:
   new_upc_preimg = cairo.ImageSurface(cairo.FORMAT_RGB24, (29 * barwidth[0]) * int(resize), (barheightadd + (9 * barwidth[1])) * int(resize));
  ...
  upc_img = new_upc_img;
 if(not hidetext):
  drawColorText(upc_img, 10 * int(resize * barwidth[1]), (5 + (6 * (int(resize) - 1))) * barwidth[0], cairo_addon_fix + (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), LeftDigit[0], barcolor[1], "ocrb", imageoutlib);
  drawColorText(upc_img, 10 * int(resize * barwidth[1]), (13 + (13 * (int(resize) - 1))) * barwidth[0], cairo_addon_fix + (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), LeftDigit[1], barcolor[1], "ocrb", imageoutlib);
 del(upc_img);
 exargdict = {};
