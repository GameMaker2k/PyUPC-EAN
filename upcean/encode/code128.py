 ...
 if(not hidetext):
  ...
  while (NumTxtZero < len(upc_print)):
   ...
   if(len(upc_print[NumTxtZero])==2):
    ...
    drawColorText(upc_img, 10 * int(resize * barwidth[1]), ((LineTxtStart + 5) + (16 * (int(resize) - 1))) * barwidth[0], cairo_addon_fix + (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), upc_print[NumTxtZero][1], barcolor[1], "ocrb", imageoutlib);
    LineTxtStart += 11 * int(resize);
   NumTxtZero += 1;
 del(upc_img);
 exargdict = {};
