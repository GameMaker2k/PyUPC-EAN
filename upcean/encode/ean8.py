...
'''
    ...
    $FileInfo: ean8.py - Last Update: 10/30/2024 Ver. 2.10.2 RC 1 - Author: cooldude2k $
'''
...
def create_ean8_barcode(upc, outfile="./ean8.png", resize=1, hideinfo=(False, False, False), barheight=(48, 54), barwidth=(1, 1), textxy=(1, 1, 1), barcolor=((0, 0, 0), (0, 0, 0), (255, 255, 255)), imageoutlib="pillow"):
    ...
    if(pilsupport and imageoutlib == "pillow"):
        ...
        upc_img.rectangle([(0, 0), (((83 * barwidth[0]) + upc_size_add) * int(resize),
    ...
    if(cairosupport and (imageoutlib == "cairo" or imageoutlib == "cairosvg")):
        if(outfileext == "SVG"):
            ...
            upc_preimg = cairo.SVGSurface(
                imgoutfile, ((83 * barwidth[0]) + upc_size_add) * int(resize), (barheightadd + (9 * barwidth[1])) * int(resize))
        elif(outfileext == "PDF"):
            upc_preimg = cairo.PDFSurface(
                None, ((83 * barwidth[0]) + upc_size_add) * int(resize), (barheightadd + (9 * barwidth[1])) * int(resize))
            ...
            upc_preimg = cairo.ImageSurface(
                cairo.FORMAT_RGB24, ((83 * barwidth[0]) + upc_size_add) * int(resize), (barheightadd + (9 * barwidth[1])) * int(resize))
        upc_img = cairo.Context(upc_preimg)
        upc_img.set_antialias(cairo.ANTIALIAS_NONE)
        upc_img.rectangle(
            0, 0, ((83 * barwidth[0]) + upc_size_add) * int(resize), (barheightadd + (9 * barwidth[1])) * int(resize))
