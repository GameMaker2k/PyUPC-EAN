...
def get_gs1_prefix(upc):
    ...
    if (not re.findall(r"^(\d{3}\d{5}|\d{3}\d{10})$", upc)):
        return False
    if (re.findall(r"^(\d{3}\d{10})$", upc)
            and not upcean.validate.validate_ean13_checksum(upc)):
        return False
    if (re.findall(r"^(\d{3}\d{5})$", upc)
            and not upcean.validate.validate_ean8_checksum(upc)):
        return False
