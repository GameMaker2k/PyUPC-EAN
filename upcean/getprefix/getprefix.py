...
'''
    ...
    $FileInfo: getprefix.py - Last Update: 8/18/2023 Ver. 2.10.0 RC 1 - Author: cooldude2k $
'''
...
def get_upca_barcode_info(upc, infotype=None):
    ...
    upc_type = {
        'packagecode': None,
        'numbersystem': pre_upc_type[0],
        'manufacturer': pre_upc_type[1],
        'product': pre_upc_type[2],
        'checkdigit': pre_upc_type[3]}
    if (infotype is None):
        return upc_type
...
def get_upca_barcode_info_from_upce(upc):
    return get_upca_barcode_info(
        upcean.convert.convert_barcode_from_upce_to_upca(upc))
...
def get_ean8_barcode_info(upc, infotype=None):
    ...
    upc_type = {
        ...
        'numbersystem': pre_upc_type[0],
        'manufacturer': None,
        'product': pre_upc_type[1],
        'checkdigit': pre_upc_type[2]}
    if (infotype is None):
        return upc_type
    if (infotype is not None):
        return upc_type.get(infotype, upc_type)
...
def get_itf14_barcode_info(upc, infotype=None):
    ...
    pre_upc_type = upc_matches[0]
    upc_type = {
        'packagecode': pre_upc_type[0],
        'numbersystem': pre_upc_type[1],
        'manufacturer': pre_upc_type[2],
        'product': pre_upc_type[3],
        'checkdigit': pre_upc_type[4]}
    if (infotype is None):
        return upc_type
    if (infotype is not None):
        return upc_type.get(infotype, upc_type)
...
def get_goodwill_upca_barcode_info(upc, infotype=None):
    ...
    formated_price_alt = str(
        price_matches[0].lstrip('0')) + "." + price_matches[1]
    product = {
        'numbersystem': str(4),
        'code': upc_matches[0],
        'price': upc_matches[1],
        'pricendnz': price_alt,
        'pricewdwz': formated_price,
        'pricewdnz': formated_price_alt,
        'type': gw_item_type,
...
def get_upca_coupon_barcode_info(upc, infotype=None):
    ...
    upc_matches = upc_matches[0]
    product = {
        'numbersystem': upc_matches[0],
        'manufacturer': upc_matches[1],
        'family': upc_matches[2],
        'value': upc_matches[3],
...
def get_ups_barcode_info(upc, infotype=None):
    ...
    pre_upc_type = upc_matches[0]
    upc_type = {
        'accountnumber': pre_upc_type[0],
        'servicetype': pre_upc_type[1],
        'invoicenumber': pre_upc_type[2],
        'packagenumber': pre_upc_type[3],
        'checkdigit': pre_upc_type[4]}
    if (infotype is None):
        return upc_type
    if (infotype is not None):
...
def get_bcn_info(upc, infotype=None):
    ...
    upc_type = {
        'mii': pre_upc_type[0],
        'iin': pre_upc_type[0] + pre_upc_type[1],
        'account': pre_upc_type[2],
        'checkdigit': pre_upc_type[3]}
    if (infotype is None):
        return upc_type
    if (infotype is not None):
...
def get_upca_ndc_barcode_info(upc, infotype=None):
    ...
    product = {
        'numbersystem': str(3),
        'labeler': upc_matches[0],
        'productcode': upc_matches[1],
        'packagecode': upc_matches[2],
        'checkdigit': upc_matches[3]}
    if (infotype is None):
        return product
    if (infotype is not None):
