...
def convert_barcode_from_upce_to_ean13(upc):
    upc = str(upc)
    return convert_barcode_from_upca_to_ean13(
        convert_barcode_from_upce_to_upca(upc))


def convert_barcode_from_upce_to_itf14(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_itf14(
        convert_barcode_from_upce_to_ean13(upc))


def convert_barcode_from_upca_to_itf14(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_itf14(
        convert_barcode_from_upca_to_ean13(upc))
...
def convert_barcode_from_ean13_to_upce(upc):
    upc = str(upc)
    return convert_barcode_from_upca_to_upce(
        convert_barcode_from_ean13_to_upca(upc))


def convert_barcode_from_itf14_to_upca(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_upca(
        convert_barcode_from_itf14_to_ean13(upc))


def convert_barcode_from_itf14_to_upce(upc):
    upc = str(upc)
    return convert_barcode_from_upca_to_upce(
        convert_barcode_from_itf14_to_upca(upc))
...
def convert_barcode_from_ean8_to_ean13(upc):
    upc = str(upc)
    return convert_barcode_from_upca_to_ean13(
        convert_barcode_from_ean8_to_upca(upc))


def convert_barcode_from_ean8_to_itf14(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_itf14(
        convert_barcode_from_ean8_to_ean13(upc))
...
def convert_barcode_from_ean13_to_ean8(upc):
    upc = str(upc)
    return convert_barcode_from_upca_to_ean8(
        convert_barcode_from_ean13_to_upca(upc))


def convert_barcode_from_itf14_to_ean8(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_ean8(
        convert_barcode_from_itf14_to_ean13(upc))
    ...
    while (textc < textlen):
        ...
        if (asciitohex.get(upc[textc], False)):
            if (
                (
                    upc[textc] == "Ä" and (
                        incharset == 1 or incharset == 2 or incharset is None)) or (
                    upc[textc] == "Å" and (
                    incharset == 1 or incharset == 2 or incharset is None)) or (
                        upc[textc] == "È" and (
                            incharset == 2 or incharset is None)) or (
                                upc[textc] == "É" and (
                                    incharset == 1 or incharset is None)) or upc[textc] == "Ê"):
                if (textc == 0):
                    textlist.append("67")
                    incharset = 2
            ...
            if (incharset == 2):
                ...
                if (textc == 0):
                    ...
                    if (((textc +
                          1) < textlen) and hextocharsettwo.get(upc[textc +
                                                                    1], False) or incharset == 3):
                        textlist.append("64")
                    else:
                        textlist.append("62")
                        shiftcharset = True
            ...
            if (incharset == 1):
                ...
                if (textc == 0):
                    ...
                    if (((textc +
                          1) < textlen) and hextocharsetone.get(upc[textc +
                                                                    1], False) or incharset == 3):
                        textlist.append("65")
                    else:
                        textlist.append("62")
...
def convert_text_to_hex_code128_with_checksum(upc):
    ...
    if (not code128out):
        return False
    return code128out + "6d" + \
        upcean.validate.get_code128_checksum(code128out) + "6c"
...
def convert_barcode_from_issn8_to_itf14(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_itf14(
        convert_barcode_from_issn8_to_issn13(upc))


def convert_barcode_from_itf14_to_issn8(upc):
    upc = str(upc)
    return convert_barcode_from_itf14_to_ean13(
        convert_barcode_from_issn13_to_issn8(upc))
...
def convert_barcode_from_isbn10_to_itf14(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_itf14(
        convert_barcode_from_isbn10_to_isbn13(upc))


def convert_barcode_from_itf14_to_isbn10(upc):
    upc = str(upc)
    return convert_barcode_from_itf14_to_ean13(
        convert_barcode_from_isbn13_to_isbn10(upc))
...
def convert_barcode_from_ismn10_to_itf14(upc):
    upc = str(upc)
    return convert_barcode_from_ean13_to_itf14(
        convert_barcode_from_ismn10_to_ismn13(upc))


def convert_barcode_from_itf14_to_ismn10(upc):
    upc = str(upc)
    return convert_barcode_from_itf14_to_ean13(
        convert_barcode_from_ismn13_to_ismn10(upc))
...
def print_ismn10_barcode(upc):
    ...
    ismn10 = "M-" + ismn_matches[0] + "-" + \
        ismn_matches[1] + "-" + ismn_matches[2]
    return ismn10
