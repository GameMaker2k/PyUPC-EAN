'''
    ...
    FileInfo: ean5.py - Last Update: 10/31/2014 Ver. 2.7.0 RC 1 - Author: cooldude2k 
'''
...
def create_ean5_barcode_supplement(upc,outfile="./ean5_supplement.png",resize=1,hideinfo=(False, False, False),barheight=(48, 54),textxy=(1, 1, 1),barcolor=((0, 0, 0), (0, 0, 0), (255, 255, 255))):
 ...
 if(hidetext==False):
  upcean.prepil.drawColorText(upc_img, 10 * int(resize), 7 + (7 * (int(resize) - 1)), (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), LeftDigit[0], barcolor[1]);
  upcean.prepil.drawColorText(upc_img, 10 * int(resize), 16 + (15 * (int(resize) - 1)), (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), LeftDigit[1], barcolor[1]);
  upcean.prepil.drawColorText(upc_img, 10 * int(resize), 24 + (24 * (int(resize) - 1)), (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), LeftDigit[2], barcolor[1]);
  upcean.prepil.drawColorText(upc_img, 10 * int(resize), 32 + (32 * (int(resize) - 1)), (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), LeftDigit[3], barcolor[1]);
  upcean.prepil.drawColorText(upc_img, 10 * int(resize), 40 + (40 * (int(resize) - 1)), (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), LeftDigit[4], barcolor[1]);
 del(upc_img);
