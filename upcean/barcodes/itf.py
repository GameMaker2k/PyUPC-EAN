'''
    ...
    $FileInfo: itf.py - Last Update: 11/26/2019 Ver. 2.7.17 RC 1 - Author: cooldude2k $
'''
...
def create_itf_barcode(upc,outfile="./itf.png",resize=1,hideinfo=(False, False, False),barheight=(48, 54),textxy=(1, 1, 1),barcolor=((0, 0, 0), (0, 0, 0), (255, 255, 255))):
 ...
 upcean.barcodes.prepil.drawColorLine(upc_img, 15, 4, 15, barheight[0], barcolor[0]);
 ...
 while (NumZero < len(upc_matches)):
  ...
  while (InnerUPCNum < len(left_barcolor)):
   ...
   InnerUPCNum += 1;
  NumZero += 1;
 upcean.barcodes.prepil.drawColorLine(upc_img, 17 + upc_size_add, 4, 17 + upc_size_add, barheight[0], barcolor[0]);
 upcean.barcodes.prepil.drawColorLine(upc_img, 18 + upc_size_add, 4, 18 + upc_size_add, barheight[0], barcolor[0]);
 upcean.barcodes.prepil.drawColorLine(upc_img, 19 + upc_size_add, 4, 19 + upc_size_add, barheight[0], barcolor[0]);
 upcean.barcodes.prepil.drawColorLine(upc_img, 20 + upc_size_add, 4, 20 + upc_size_add, barheight[0], barcolor[2]);
 upcean.barcodes.prepil.drawColorLine(upc_img, 21 + upc_size_add, 4, 21 + upc_size_add, barheight[0], barcolor[0]);
 upcean.barcodes.prepil.drawColorLine(upc_img, 22 + upc_size_add, 4, 22 + upc_size_add, barheight[0], barcolor[2]);
 ...
 if(not hidetext):
  ...
  while (NumTxtZero < len(upc_matches)):
   ...
   upcean.barcodes.prepil.drawColorText(upc_img, 10 * int(resize), LineTxtStart + (21 * (int(resize) - 1)), (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), ArrayDigit[0], barcolor[1]);
   LineTxtStart += 9 * int(resize);
   upcean.barcodes.prepil.drawColorText(upc_img, 10 * int(resize), LineTxtStart + (21 * (int(resize) - 1)), (barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix) + (textxy[1] * int(resize)), ArrayDigit[1], barcolor[1]);
   LineTxtStart += 9 * int(resize);
   NumTxtZero += 1;
 del(upc_img);
