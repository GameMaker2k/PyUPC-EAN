...
def decode_ean13_barcode(
    ...
    while (listcount < countlist):
        if (listcount < 6):
            if (left_barcode_l_dict.get(
                    pre_upc_whole[listcount], None) is not None):
...
def get_gtin13_barcode_location(
    infile="./gtin13.png", resize=1, barheight=(
        48, 54), barwidth=(
            1, 1), shiftcheck=False, shiftxy=(
                0, 0), barcolor=(
                    (0, 0, 0), (0, 0, 0), (255, 255, 255)), imageoutlib="pillow"):
    return decode_gtin13_barcode(
        infile,
        resize,
        barheight,
        barwidth,
...
def get_ucc13_barcode_location(
    infile="./ucc13.png", resize=1, barheight=(
        48, 54), barwidth=(
            1, 1), shiftcheck=False, shiftxy=(
                0, 0), barcolor=(
                    (0, 0, 0), (0, 0, 0), (255, 255, 255)), imageoutlib="pillow"):
    return decode_ucc13_barcode(
        infile,
        resize,
        barheight,
        barwidth,
