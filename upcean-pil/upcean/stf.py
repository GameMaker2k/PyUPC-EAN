'''
    ...
    $FileInfo: stf.py - Last Update: 11/23/2013 Ver. 2.5.0 RC 1 - Author: cooldude2k $
'''
...
def create_stf(upc,outfile="./stf.png",resize=1,hideinfo=(False, False, False),barheight=(48, 54),barcolor=((0, 0, 0), (0, 0, 0), (255, 255, 255))):
 ...
 if(hidetext==False):
  ...
  while (NumTxtZero < len(upc_matches)):
   drawColorText(upc_img, 10 * int(resize), LineTxtStart + (24 * (int(resize) - 1)), barheight[0] + (barheight[0] * (int(resize) - 1)) + pil_addon_fix, upc_matches[NumTxtZero], barcolor[1]);
