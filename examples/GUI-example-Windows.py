...
class MainWin(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.frameMain.grid(row=1, column=0, columnspan=2,
                            sticky='nswe', padx=(10, 0), pady=(0, 10))
        ...
        self.frameType.grid(row=0, column=0, sticky="nswe",
                            padx=(0, 0), pady=(2, 2))
        ttk.Label(self.frameType, text='Barcode type:  ').grid(
        ...
        self.frameSize = ttk.LabelFrame(
            self.frameTopLeft, text="Barcode size:", padding=(3, 3))
        ...
        self.bcode_val = ttk.Entry(
            self.frameTopLeft, width=16, textvariable=self.bcvalue, font=('Arial', 15))
        self.bcode_val.grid(row=3, column=0, pady=(0, 0))

        ttk.Label(self.frameTopLeft, text='Comment:').grid(
            row=4, column=0, sticky='w', padx=(5, 0), pady=(5, 0))
        ...
        self.bcode_comment = tkst.ScrolledText(
            self.textFrame, wrap=tk.WORD, width=23, height=3, font=('Arial', 10))
        ...
        self.frameButton.grid(
            row=0, column=0, sticky="nswe", padx=(0, 5), pady=(5, 5))
        ...
        self.btnGenerate = ttk.Button(
            self.frameButton, text='Generate', command=self.generate)
        self.btnGenerate.grid(row=0, column=0, pady=(
            0, 0), padx=(0, 2), sticky="we")
        self.btnSave = ttk.Button(
            self.frameButton, text='Save', command=self.saveasfile)
        self.btnSave.grid(row=0, column=1, pady=(0, 0),
                          padx=(2, 0), sticky="we")
        ...
        self.imagepanel.config(yscrollcommand=lambda f,
                               l: self.autoscroll(self.vsb, f, l))
        self.imagepanel.config(xscrollcommand=lambda f,
                               l: self.autoscroll(self.hsb, f, l))
        self.imagepanel.grid(row=1, column=0, sticky="nswe",
                             padx=(0, 5), pady=(0, 5))
        ...
        self.hsb1 = ttk.Scrollbar(
            self.frameMain, orient="horizontal", command=self.tree.xview)
        self.vsb1.grid(row=0, column=1, sticky="nse")
        self.hsb1.grid(row=1, column=0, sticky="sew")
        self.tree.config(yscrollcommand=lambda f,
                         l: self.autoscroll(self.vsb1, f, l))
        self.tree.config(xscrollcommand=lambda f,
                         l: self.autoscroll(self.hsb1, f, l))
        self.tree.heading("#0", text="#")
        self.tree.heading("barcodes", text="Barcodes")
        self.tree.heading("type", text="Type")
        self.tree.heading("comment", text="Comment")

        self.tree.column("#0", minwidth=20, width=50,
                         stretch=True, anchor="center")
        self.tree.column("barcodes", minwidth=50, width=85,
                         stretch=True, anchor="center")
        self.tree.column("type", minwidth=40, width=65,
                         stretch=True, anchor="center")
        self.tree.column("comment", minwidth=120, width=245,
                         stretch=True, anchor="center")
    ...
    def read_file(self):
        if os.path.isfile('data.csv'):
            try:
                ...
                mbox.showerror(
                    "Xatolik", "Error occured while loading Data.csv!")
...
class SettingWin(tk.Frame):
    ...
    def __init__(self, master):
        ...
        ttk.Label(self.frameMain, text='Barcode type: ').grid(
        ...
        self.dcfiletype = ttk.OptionMenu(
            self.frameMain, self.default_filetype, filetypes[0], *filetypes)
        ...
        self.dean13 = ttk.Entry(self.frameMain, width=33,
        ...
        ttk.Label(self.frameMain, text='EAN-08 initial value:').grid(row=5,
                                                                     column=0, columnspan=2, sticky='w', pady=(5, 0))
        self.dean08 = ttk.Entry(self.frameMain, width=33,
        ...
        ttk.Label(self.frameMain, text='EAN-05 initial value:').grid(row=7,
                                                                     column=0, columnspan=2, sticky='w', pady=(5, 0))
        self.dean05 = ttk.Entry(self.frameMain, width=33,
        ...
        ttk.Label(self.framepdf, text='File saving directory:  ', font=(
            "Arial", 9)).grid(row=0, column=0, sticky='w', pady=(5, 0))
        self.dfiledir = ttk.Entry(self.framepdf, width=23, font=(
            "Arial", 9), textvariable=self.default_dir)
        self.dfiledir.grid(row=1, column=0, padx=(3, 0), pady=(5, 0))
        self.pdfbtn = ttk.Button(
            self.framepdf, text='...', width=3, command=self.folder)
        self.pdfbtn.grid(row=1, column=1, padx=(5, 0), pady=(5, 0))

        self.skpSave = ttk.Button(
            self.bottom, text='Save', command=self.save_list)
        self.skpSave.grid(row=0, column=0, sticky='we',
                          padx=(0, 5), pady=(10, 3))
        self.btcancel = ttk.Button(
            self.bottom, text='Cancel', command=self.cancel)
        self.btcancel.grid(row=0, column=1, sticky='we',
                           padx=(5, 0), pady=(10, 3))
...
class HelpWin(tk.Frame):
    ...
    def __init__(self, master):
        ...
        ttk.Label(self.top, text='Github: github.com/bzimor').grid(row=3,
                                                                   column=0, padx=(10, 10), pady=(5, 0), sticky='nswe')
        ttk.Label(self.top, text='Email: bobzimor@gmail.com').grid(row=4,
                                                                   column=0, padx=(10, 10), pady=(5, 0), sticky='nswe')
