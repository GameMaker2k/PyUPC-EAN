...
rootwin = Tk()
rootwin.wm_title(str(pro_app_name) + str(pro_app_subname) +
                 " - Version: " + str(pro_app_version))
...
def hex_color_to_tuple(color):
    ...
    if (re.findall(
        ...
        return (int(pre_color[0], 16), int(
            pre_color[1], 16), int(pre_color[2], 16))
    return False
...
def tuple_color_to_hex(color):
    if (not isinstance(color, tuple) or not len(color) == 3):
        return False
    return "#" + hex(int(color[0])).replace("0x",
                                            "").upper().zfill(2) + hex(int(color[1])).replace("0x",
                                                                                              "").upper().zfill(2) + hex(int(color[2])).replace("0x",
                                                                                                                                                "").upper().zfill(2)
...
def show_ccp_menu(e):
    ...
    the_menu.entryconfigure("Delete", command=lambda: w.delete(
        SEL_FIRST, SEL_LAST) if w.select_present() else None)
    the_menu.entryconfigure("Delete All", command=lambda: w.delete(0, END))
    the_menu.entryconfigure(
        "Select All", command=lambda: w.selection_range(0, END))
...
def ch_barcode_colors():
    ...
    tkMessageBox.showinfo("Background Color",
                          "Select a color for the background.")
    barcode_bg_color = hex_color_to_tuple(tkColorPicker(
        tuple_color_to_hex(barcode_bg_color), "Background Color"))
    tkMessageBox.showinfo("Bar Color", "Select a color for the bars.")
    barcode_bar_color = hex_color_to_tuple(tkColorPicker(
        tuple_color_to_hex(barcode_bar_color), "Bar Color"))
    tkMessageBox.showinfo("Bar Color", "Select a color for the text.")
    barcode_text_color = hex_color_to_tuple(tkColorPicker(
        tuple_color_to_hex(barcode_text_color), "Text Color"))
    GenerateBarcode()
...
if (sys.platform == "linux" or sys.platform == "linux2" or sys.platform ==
        "bsdos" or sys.platform == "freebsd" or sys.platform == "netbsd"):
...
def GenerateBarcode():
    ...
    if (not entry2.get().isdigit() or not entry3.get().isdigit()):
        tkMessageBox.showerror("PyUPC-EAN - Error",
                               "Bar Height has to be a integer value.")
    ...
    if (not magnify.get().isdigit()):
        tkMessageBox.showerror("PyUPC-EAN - Error",
                               "Magnify has to be a integer value.")
        magnify.delete(0, END)
        magnify.insert(0, "1")
    upc_validate = entry1.get()
    if (listboxtxt1.get() == "UPC-A" or listboxtxt1.get() ==
            "UPC-E" or listboxtxt1.get() == "EAN-13" or listboxtxt1.get() == "EAN-8"):
    ...
    tmpbarcode = upcean.oopfuncs.barcode(
        barcode_list[listboxtxt1.get()], entry1.get())
    tmpbarcode.size = 2
    ...
    if (not validbc):
        ...
        rootwin.wm_title(str(pro_app_name) + str(pro_app_subname) +
                         " - Version: " + str(pro_app_version))
    if (validbc):
        rootwin.wm_title(str(pro_app_name) +
                         str(pro_app_subname) + " - " + str(entry1.get()))
        ...
        panel1 = Canvas(imageframe1, xscrollcommand=xscrollbar1.set,
                        width=350, height=validbc.size[1])
        panel1.bind_class(
            "Canvas", "<Button-3><ButtonRelease-3>", show_save_menu)
        panel1.create_image(
            validbc.size[0] / 2,
            validbc.size[1] / 2,
            image=image1)
