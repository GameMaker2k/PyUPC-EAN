...
'''
    ...
    $FileInfo: itf.py - Last Update: 10/04/2012 Ver. 2.0.0 - Author: cooldude2k $
'''
...
def create_itf(upc,outfile="./itf.png",resize=1,hideinfo=(False, False, False),barheight=(48, 54)):
 upc = str(upc);
 hidesn = hideinfo[0];
 hidecd = hideinfo[1];
 hidetext = hideinfo[2];
 ...
 if(len(upc_matches)<=0):
  return False;
 upc_preimg = Image.new("RGB", (39 + upc_size_add, barheight[0] + 14));
 upc_img = ImageDraw.Draw(upc_preimg);
 upc_img.rectangle([(0, 0), (39 + upc_size_add, barheight[0] + 14)], fill=(256, 256, 256));
 text_color = (0, 0, 0);
 ...
 drawColorLine(upc_img, 15, 4, 15, barheight[0], text_color);
 ...
 while (NumZero < len(upc_matches)):
  ...
  while (InnerUPCNum < len(left_text_color)):
   ...
   InnerUPCNum += 1;
  NumZero += 1;
 drawColorLine(upc_img, 17 + upc_size_add, 4, 17 + upc_size_add, barheight[0], text_color);
 drawColorLine(upc_img, 18 + upc_size_add, 4, 18 + upc_size_add, barheight[0], text_color);
 drawColorLine(upc_img, 19 + upc_size_add, 4, 19 + upc_size_add, barheight[0], text_color);
 drawColorLine(upc_img, 20 + upc_size_add, 4, 20 + upc_size_add, barheight[0], alt_text_color);
 drawColorLine(upc_img, 21 + upc_size_add, 4, 21 + upc_size_add, barheight[0], text_color);
 drawColorLine(upc_img, 22 + upc_size_add, 4, 22 + upc_size_add, barheight[0], alt_text_color);
 ...
 new_upc_img.paint();
 '''
 new_upc_img = upc_preimg.resize(((39 + upc_size_add) * int(resize), (barheight[0] + 14) * int(resize)), Image.BILINEAR) # linear interpolation in a 2x2 environment
 '''
 new_upc_img = upc_preimg.resize(((39 + upc_size_add) * int(resize), (barheight[0] + 14) * int(resize)), Image.NEAREST) # use nearest neighbour
 new_upc_img = upc_preimg.resize(((39 + upc_size_add) * int(resize), (barheight[0] + 14) * int(resize)), Image.BICUBIC) # cubic spline interpolation in a 4x4 environment
 new_upc_img = upc_preimg.resize(((39 + upc_size_add) * int(resize), (barheight[0] + 14) * int(resize)), Image.ANTIALIAS) # best down-sizing filter
 '''
 del(upc_img);
 del(upc_preimg);
